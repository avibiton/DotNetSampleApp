...
:childTestExecution
%SL_AGENT% testListener --logAppendFile true --tokenfile sltoken.txt --buildSessionIdFile buildSessionId.txt --target "packages\xunit.runner.console.2.2.0\tools\xunit.console.exe" --targetArgs "MyCalculatorXUnitTests\bin\Debug\MyCalculatorXUnitTests.dll -xml xunit-report.xml" --proxy http://127.0.0.1:8888
goto endExecution

:sideBySideTestExecution
%SL_AGENT% startBackgroundTestListener --logAppendFile true --tokenfile sltoken.txt --buildSessionIdFile buildSessionId.txt --proxy http://127.0.0.1:8888 --testListenerSessionKey %SESSION_KEY%
...
%SL_AGENT% stopBackgroundTestListener --logAppendFile true --tokenfile sltoken.txt --buildSessionIdFile buildSessionId.txt --proxy http://127.0.0.1:8888 --testListenerSessionKey %SESSION_KEY%
...
%SL_AGENT% reportParser --tokenfile sltoken.txt --buildSessionIdFile buildSessionId.txt --report xunit-report.xml --testStage %SL_ENV_NAME% --proxy http://127.0.0.1:8888
